...
// Same helper used in appointments page — keeps logic consistent
...
function SkeletonRow() {
  return <div className="h-14 mx-5 my-2 bg-slate-100 rounded-xl animate-pulse" />;
}
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setProfile(p);
      setScans(s);
      // Only future / today appointments, not cancelled
      setAppointments(a.filter((apt) => apt.status !== "cancelled" && isUpcoming(apt.date)));
      setLoading(false);
    }
    void load();
  }, []);
  ...
  const upcomingAppt = appointments.slice(0, 2);
  ...
  const nextApptLabel = appointments[0]
  ...
  const stats = [
    ...
    { label: "Appointments",  value: String(appointments.length),  sub: `Next: ${nextApptLabel}`             },
  ];
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {quickActions.map((a) => (
          ...
          >
            ...
            <div>
              <p className={`text-[13px] font-semibold ${a.dark ? "text-white" : "text-gray-900"}`}>
                {a.title}
              </p>
              <p className={`text-[11px] mt-0.5 ${a.dark ? "text-white/40" : "text-gray-400"}`}>
                {a.desc}
              </p>
            </div>
            <MdArrowForward
              ...
              className={`mt-auto transition-colors ${
                a.dark
                  ...
                  : "text-gray-300 group-hover:text-[#185FA5]"
              }`}
            />
          </Link>
        ))}
      ...
      <div className="grid md:grid-cols-2 gap-5">
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <Link href="/dashboard/history" className="text-[11px] text-[#185FA5] hover:underline font-medium">
              View all
            </Link>
          </div>
          <div className="divide-y divide-slate-50">
            {loading ? (
              ...
              <div className="px-5 py-8 text-center">
                ...
                <Link href="/dashboard/scan" className="text-[12px] font-semibold text-[#185FA5] hover:underline">
                  Run your first scan →
                </Link>
              </div>
            ) : (
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            ...
            <Link href="/dashboard/appointments" className="text-[11px] text-[#185FA5] hover:underline font-medium">
              View all
            </Link>
          </div>
          <div className="divide-y divide-slate-50">
            {loading ? (
              ...
              <div className="px-5 py-8 text-center">
                ...
                <Link href="/dashboard/appointments" className="text-[12px] font-semibold text-[#185FA5] hover:underline">
                  Book one →
                </Link>
              </div>
            ) : (
              upcomingAppt.map((apt) => (
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[11px] text-white/40 mt-0.5">
              Every 20 min of screen time, look 20 ft away for 20 seconds.
            </p>
          </div>
        </div>
        <Link
